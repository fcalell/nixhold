...
| Item | Trigger | Intended shape |
|---|---|---|
| A shared provisioning-unit helper | the second framework consumer of "Provisioning" (the repository checkout is the first; the rendering lives in `modules/repositories`) | one function from `{ name; script; done; }` to the systemd user service (exec, on-failure every 30 s, bounded start limit, `ConditionPathExists=!done`) and the launchd agent (`KeepAlive.SuccessfulExit = false`, `ThrottleInterval = 30`, the script gating on `done`), so two consumers cannot drift on the retry shape; a fleet's own network-touching provisioning (a runtime-fetched binary) is its natural third |
