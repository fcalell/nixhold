...
| Concurrent `deploy --all` | the fleet's second remote machine | every remote build is the target's own (ARCHITECTURE "Where a host is built"), so they can run at once: sudo passwords gathered up front, output prefixed per host, activation still in order |
| A kexec image of the fleet installer | the first VPS | `host install --remote` clones and builds on the target, so the target needs git, nix, disko and the pinned forge keys — what the fleet ISO carries and nixos-anywhere's kexec image does not (it ships no git). The image is the ISO's module built as a kexec tarball, and the verb kexecs a VPS into it before the phases it runs today |
| `status` compares the deployed sha with HEAD and the forge | first "which commit is this host on?" that the sha alone does not answer | `status` prints `system.configurationRevision` today; the comparison is the missing half |
