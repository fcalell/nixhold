...
| A routed endpoint on a host with no caddy | met: the dogfood fleet's desktop enables syncthing, whose `gui` endpoint names the tailnet, and `desktopLinux` imports neither `infra.caddy` nor `infra.firewall`, so the endpoint was a silent no-op until the host imported both | **decision**: the infra modules move to the NixOS baseline, the way `infra.backups` did (they activate from data, so a host with no endpoint renders nothing), or an assertion in `infra/endpoints.nix` refuses an endpoint on a host that imported no terminator |
